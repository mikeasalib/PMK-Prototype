...
function WhatsImportant() {
  ...
  const blocked = linear.filter((i) => {
    ...
    return t.includes("blocked") || t.includes("blocker") || (i.labels ?? []).some((l) => /block/i.test(l));
  });
  const urgent = linear
...
function Kpi({ label, value, sub, danger }: { label: string; value: string; sub: string; danger?: boolean }) {
  return (
    <div
      ...
      style={{ border: "1px solid #e5e5e2", borderLeft: `3px solid ${danger ? "#b3261e" : "#005ea2"}` }}
    >
      <div className="text-[10px] uppercase tracking-wide" style={{ color: "#565c65" }}>{label}</div>
      <div className="text-2xl font-bold" style={{ fontFamily: 'Public Sans, system-ui, sans-serif', color: danger ? "#b3261e" : "#3a5a40" }}>
        {value}
      </div>
      <div className="text-[11px]" style={{ color: "#565c65" }}>{sub}</div>
    </div>
  );
}
...
function Panel({ title, children, accent }: { title: string; children: React.ReactNode; accent: string }) {
  return (
    <section className="rounded-md bg-white" style={{ border: "1px solid #e5e5e2", borderTop: `3px solid ${accent}` }}>
      <div className="px-3 py-2 text-[12px] font-semibold" style={{ color: "#3a5a40", borderBottom: "1px solid #eee", fontFamily: 'Public Sans, system-ui, sans-serif' }}>
        {title}
      </div>
...
function Empty({ text }: { text: string }) {
  return <div className="py-4 text-center text-[12px]" style={{ color: "#565c65" }}>{text}</div>;
}
  ...
  return (
    <li className="py-2">
      ...
      <div className="mt-1 flex flex-wrap items-center gap-2 text-[11px]" style={{ color: "#565c65" }}>
        <span className="rounded px-1.5 py-0.5 font-semibold" style={{ color: wsColor, backgroundColor: `${wsColor}14`, border: `1px solid ${wsColor}33` }}>
          {ws}
        </span>
        <span>{issue.assignee ?? "unassigned"}</span>
        <span>· {issue.state_name ?? "—"}</span>
        {showPriority ? (
          <span className="rounded px-1 py-0.5 font-semibold" style={{ color: priorityColor(issue.priority), backgroundColor: `${priorityColor(issue.priority)}14`, border: `1px solid ${priorityColor(issue.priority)}44` }}>
...
function MilestoneRow({ label, date, danger }: { label: string; date: string; danger?: boolean }) {
  ...
  return (
    <li className="flex items-center justify-between">
      <span>{label}</span>
      <span className="font-mono" style={{ color: danger ? "#b3261e" : "#3a5a40", fontWeight: danger ? 700 : 500 }}>
        {d > 0 ? `${d}d` : d === 0 ? "today" : `${-d}d ago`} · {new Date(date).toLocaleDateString(undefined, { month: "short", day: "numeric" })}
      </span>
    </li>
  );
}
